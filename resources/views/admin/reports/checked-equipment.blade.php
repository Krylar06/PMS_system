...
<div class="space-y-5">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Equipment marked checked individually by admin accounts.</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        @forelse($adminSummary->take(3) as $summary)
            <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div class="text-sm font-medium text-gray-500">{{ $summary->checkedBy?->name ?? 'Unknown Admin' }}</div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <form method="GET" class="grid grid-cols-1 gap-3 lg:grid-cols-6">
            ...
            <select name="admin_id" class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
                <option value="">All admin accounts</option>
                @foreach($adminUsers as $admin)
                    <option value="{{ $admin->id }}" @selected((int) $adminId === $admin->id)>{{ $admin->name }}</option>
                @endforeach
            </select>
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-gray-200 px-5 py-4">
            ...
            <button onclick="window.print()" class="rounded-xl bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-black">Print</button>
        </div>
            ...
            <table class="min-w-full text-left text-sm">
                <thead class="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                    <tr>
                        ...
                        <th class="px-4 py-3">Admin Account</th>
                        <th class="px-4 py-3">Device</th>
                        <th class="px-4 py-3">Type</th>
                        <th class="px-4 py-3">Office / College</th>
                        <th class="px-4 py-3">Maintenance Type</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($records as $record)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-700">{{ $record->maintenance_type ?: '-' }}</td>
